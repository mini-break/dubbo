...
final class HeartBeatTask implements Runnable {
     ...
     * 1.如果需要心跳的通道本身关闭了，那么跳过，不添加心跳机制。
     * 2.无论是接收消息还是发送消息，只要超过了设置的心跳间隔，就发送心跳消息来测试是否断开
     * 3.如果最后一次接收到消息到现在已经超过了心跳超时时间，那就认定对方的确断开，分两种情况来处理对方断开的情况。
        ...
        try {
            ...
            for (Channel channel : channelProvider.getChannels()) {
                ...
                try {
                    ...
                    // 如果最后一次接收或者发送消息的时间到现在的时间间隔超过了心跳间隔时间
                    ...
                    if (lastRead != null && now - lastRead > heartbeatTimeout) {
                        ...
                        if (channel instanceof Client) {
                            ...
                            // 如果不是客户端，也就是服务端返回响应给客户端，但是客户端挂掉了，则服务端关闭客户端连接
                            channel.close();
                        }
                    }
                } catch (Throwable t) {
